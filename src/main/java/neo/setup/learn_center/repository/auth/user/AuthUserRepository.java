package neo.setup.learn_center.repository.auth.user;
...
public interface AuthUserRepository extends JpaRepository<AuthUser, Integer>, BaseRepository {
    ...
    @Query(value = "select r.name from auth.role r " +
            "inner join auth.auth_user_role aur on r.id = aur.role_id " +
            "inner join auth.auth_user au on aur.user_id = au.id where au.is_deleted = false and au.code = :code", nativeQuery = true)
